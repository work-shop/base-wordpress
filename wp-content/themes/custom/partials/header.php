...
<head>
	...
	if( get_field('social_media_image') ):
		...
		if( is_singular('event_listing')):
			...
			$social_image = get_event_resized_image( $banner, 'fb' ); 
		else:
			$social_image = '';
		endif;
	endif;
	...
	//endif;
		...
		$acceptLang = ['en'];
		if( in_array($lang, $acceptLang) == false ): 
			$translate_classes = ' translate-included ';
		else: 
			$translate_classes = ' english ';
		endif;
		$translate_classes = ' translation-disabled ';
		?>

	</head>
	<body <?php body_class('loading before-scroll modal-off menu-closed dropdown-off mobile-dropdown-off curve-off ' . $sitewide_alert_class . ' ' . $logged_in_classes . ' ' . $translate_classes . ' '); ?>>
